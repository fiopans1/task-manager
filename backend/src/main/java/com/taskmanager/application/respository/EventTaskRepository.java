...
public interface EventTaskRepository extends JpaRepository<EventTask, Long> {
    ...
    @Query("SELECT new com.taskmanager.application.model.dto.EventTaskDTO(e.id, t.nameOfTask, e.startTime, e.endTime) "
            + "FROM EventTask e JOIN e.task t WHERE t.user.id = :userId AND e.startTime >= :now ORDER BY e.startTime ASC")
    public List<EventTaskDTO> findUpcomingEventsByUserId(Long userId, Date now, Pageable pageable);
