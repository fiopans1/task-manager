...
class BuildTaskManager:
    ...
    def create_caddyfile(self):
        """Crea un Caddyfile básico para servir la aplicación."""
        caddyfile_content = """# Caddyfile para Task Manager
:8080 {
    ...
    # SPA fallback - redirigir todas las rutas no encontradas al index.html
...
# Configuración HTTPS opcional (descomentар para usar)
# your-domain.com {
#     root * /lib/frontend
#     file_server
#     reverse_proxy /api/* localhost:8081
...
REM Iniciar Caddy en segundo plano
echo Iniciando servidor web Caddy...
start /B bin\\caddy.exe run --config config\\Caddyfile --adapter caddyfile > logs\\caddy.log 2>&1

REM Esperar un momento para que Caddy se inicie
timeout /t 3 /nobreak > nul

REM Iniciar aplicación Java
echo Iniciando aplicación Task Manager...
java -jar lib\\backend\\taskmanager.jar --spring.config.location=config\\application.properties

REM Si llegamos aquí, la aplicación Java se cerró
echo Deteniendo Caddy...
taskkill /F /IM caddy.exe > nul 2>&1

echo Task Manager detenido.
pause
"""
...
echo "Iniciando servidor web Caddy..."
./bin/caddy run --config config/Caddyfile --adapter caddyfile > logs/caddy.log 2>&1 &
CADDY_PID=$!

# Esperar un momento para que Caddy se inicie
sleep 3

# Verificar que Caddy se inició correctamente
if ! kill -0 $CADDY_PID 2>/dev/null; then
    echo "Error: No se pudo iniciar Caddy"
    exit 1
fi
...
echo "Iniciando aplicación Task Manager..."
java -jar lib/backend/taskmanager.jar --spring.config.location=config/application.properties &
JAVA_PID=$!

echo "Task Manager iniciado (PID: $JAVA_PID)"
echo "Accede a la aplicación en: http://localhost:8080"
echo "Presiona Ctrl+C para detener la aplicación"

# Esperar a que termine la aplicación Java
wait $JAVA_PID
"""

        return windows_script, unix_script
    ...
    def compress_folder_deploy(self):
        ...
        shutil.make_archive(self.project_root / self.name_final_file, 'zip', root_dir=self.deploy_dir)
    ...
    def copy_files_after_deploy(self):
        ...
        bin_files_dir = self.scripts_dir / 'bin_files'
        if bin_files_dir.exists():
            shutil.copytree(bin_files_dir, self.deploy_bin_dir, dirs_exist_ok=True)
        
        config_files_dir = self.scripts_dir / 'config_files'
        if config_files_dir.exists():
            shutil.copytree(config_files_dir, self.deploy_config_dir, dirs_exist_ok=True)
    ...
    def deploy(self):
        ...
        (self.deploy_dir / 'logs').mkdir(parents=True)
        ...
        # Copiar Caddy al directorio bin
        caddy_dest = self.deploy_bin_dir / caddy_executable.name
        shutil.copy2(caddy_executable, caddy_dest)
        logger.info(f"Caddy copiado a: {caddy_dest}")

        # Crear Caddyfile
        caddyfile_content = self.create_caddyfile()
        caddyfile_path = self.deploy_config_dir / 'Caddyfile'
        with open(caddyfile_path, 'w', encoding='utf-8') as f:
            f.write(caddyfile_content)
        logger.info(f"Caddyfile creado en: {caddyfile_path}")

        # Crear scripts de inicio
        logger.info("Creando scripts de inicio...")
        windows_script, unix_script = self.create_startup_scripts()
        
        # Script de Windows
        with open(self.deploy_dir / 'start.bat', 'w', encoding='utf-8') as f:
            f.write(windows_script)
        
        # Script de Unix (Linux/Mac)
        start_sh_path = self.deploy_dir / 'start.sh'
        ...
        logger.info("Copiando ficheros necesarios para la instalación...")
        self.copy_files_after_deploy()

        # Crear README con instrucciones
        readme_content = """# Task Manager

## Instrucciones de uso

### Windows
Ejecuta `start.bat` para iniciar la aplicación.

### Linux/Mac
Ejecuta `./start.sh` para iniciar la aplicación.

## Acceso
Una vez iniciado, accede a la aplicación en: http://localhost:8080

## Estructura
- `bin/`: Contiene los ejecutables (Caddy)
- `lib/`: Contiene la aplicación (backend JAR y frontend)
...
## Puertos
- Puerto 8080: Servidor web (Caddy)
- Puerto 8081: Backend API (Spring Boot)
...
def main():
    ...
    parser.add_argument('--caddy-version', default='v2.7.6',
                       help='Versión de Caddy a descargar')

    args = parser.parse_args()

    # Obtener el directorio del script actual
    script_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    build_manager = BuildTaskManager(script_dir, name_jar_file=args.name_jar_file, name_final_file=args.name_final_file)
    
    # Configurar versión de Caddy si se especifica
    build_manager.caddy_version = args.caddy_version
